...
function getCalculationTypeLabel(item){
  const value = item && (item.reExport ?? item.reExportType ?? item.type ?? 'standard');
  const normalized = String(value || 'standard').toLowerCase();
  return normalized === 'reexport' || normalized === 're_export' || normalized === 're-export' || normalized === 'true' ? 'RE-EXPORT' : 'Standard';
}
...
function calc(){
  ...
  const isReExport = $('reExportType') ? $('reExportType').value === 'reExport' : false;
  const subtotal=factory+transport;
  const customsRate = productType === 'baterija' ? 0.064 : 0.05;
  const customs = isReExport ? 0 : subtotal * customsRate;
  const ecoBattery = isReExport ? 0 : batteryWeight * 0.2;
  const ecoCharger = isReExport ? 0 : chargerWeight * 0.6;
  ...
  if (productType === 'baterija') {
    ...
    customsCharger = isReExport ? 0 : chargerBase * 0.12; // 12% customs on charger price + transport
  ...
  setIfVal(['eco_b','eco'], (isReExport ? 0 : ecoBattery).toFixed(2));
  if ($('ecoCharger')) $('ecoCharger').value = (isReExport ? 0 : ecoCharger).toFixed(2);
  setIfVal(['customs_b','customs'], (isReExport ? 0 : customs).toFixed(2));
  ...
  if ($('customsCharger')) $('customsCharger').value = (isReExport ? 0 : customsCharger).toFixed(2);
  // show battery customs in costs column (customs_b) if present
  if ($('customs_b')) $('customs_b').value = (isReExport ? 0 : customs).toFixed(2);
...
function getHistoryTableHtml(type, label){
  const query = ($('search') ? $('search').value : '').trim().toLowerCase();
  const filtered = calculations.slice().reverse().filter(item => {
    const productMatches = String(item.productType || 'viljuskar') === type;
    const haystack = [item.model, item.customer, item.commercialist, item.supplier].join(' ').toLowerCase();
    const searchMatch = !query || haystack.includes(query);
    return productMatches && searchMatch;
  });

  const isBattery = type === 'baterija';
  const columns = isBattery
    ? ['Broj', 'Datum', 'Model', 'Kupac', 'Komercijalista', 'Vrsta kalkulacije', 'Cena baterije', 'Cena punjača', 'Cena set', 'Marža set', 'Akcija']
    : ['Broj', 'Datum', 'Model', 'Kupac', 'Komercijalista', 'Vrsta kalkulacije', 'Nabavna', 'Prodajna', 'RUC', 'Marža', 'Akcija'];
  ...
  const rows = filtered.map(x => {
    if (isBattery) {
      return `
        <tr>
          ...
          <td><b>${x.model || '—'}</b></td>
          <td>${x.customer || '—'}</td>
          <td>${x.commercialist || '—'}</td>
          <td>${getCalculationTypeLabel(x)}</td>
          <td>${euro(x.purchase ?? 0)}</td>
          <td>${euro(x.purchaseCharger ?? 0)}</td>
          <td>${euro(x.purchaseSet ?? x.purchaseWith ?? 0)}</td>
          <td>${((x.actualSet ?? x.actual ?? 0)).toFixed(1)}%</td>
          <td><button class="edit-btn" data-id="${x.id}">Uredi</button> <button class="print-btn" data-id="${x.id}">Štampaj</button> <button class="delete-btn" data-id="${x.id}">Obriši</button></td>
        </tr>
      `;
    }
    ...
    return `
      <tr>
        ...
        <td>${euro(x.purchase ?? 0)}</td>
        <td>${euro(x.sale ?? 0)}</td>
        <td>${euro(x.profit ?? 0)}</td>
        <td>${(Number(x.actual ?? 0)).toFixed(1)}%</td>
        <td><button class="edit-btn" data-id="${x.id}">Uredi</button> <button class="print-btn" data-id="${x.id}">Štampaj</button> <button class="delete-btn" data-id="${x.id}">Obriši</button></td>
      </tr>
    `;
  }).join('');

  const isVisible = historyFilter === 'all' || historyFilter === type;
  if (!isVisible) return '';

  const noDataText = isBattery ? 'Nema baterijskih kalkulacija za trenutno filtriranje.' : 'Nema viljuškar kalkulacija za trenutno filtriranje.';

  return `
    <div class="history-product-group" style="margin-top:16px;">
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>${rows || `<tr><td colspan="${columns.length}">${noDataText}</td></tr>`}</tbody>
        </table>
      </div>
    </div>
  `;
...
function renderHistoryTable(){
  const historyTables = $('historyTables');
  if (!historyTables) return;

  historyTables.innerHTML = [
    getHistoryTableHtml('viljuskar', 'Viljuškar'),
    getHistoryTableHtml('baterija', 'Baterija')
  ].filter(Boolean).join('');
}

function getDashboardTableHtml(type, label){
  const items = calculations.filter(item => String(item.productType || 'viljuskar') === type).slice(-8).reverse();

  const isBattery = type === 'baterija';
  const columns = isBattery
    ? ['Datum', 'Model', 'Kupac', 'Komercijalista', 'Vrsta kalkulacije', 'Cena baterije', 'Cena punjača', 'Cena set', 'Marža set', 'Akcija']
    : ['Datum', 'Model', 'Kupac', 'Komercijalista', 'Vrsta kalkulacije', 'Nabavna', 'Prodajna', 'RUC', 'Marža', 'Akcija'];
  ...
  const noDataText = isBattery ? 'Još nema sačuvanih baterijskih kalkulacija.' : 'Još nema sačuvanih viljuškar kalkulacija.';

  return `
    <div class="dashboard-product-group" style="margin-top:16px;">
      <h3 style="margin:0 0 8px;">${label}</h3>
      <div class="table-wrap">
        <table>
          <thead>
...
function render(){
 ...
 const dashboardTables = $('dashboardTables');
 if (dashboardTables) {
   dashboardTables.innerHTML = [
     getDashboardTableHtml('viljuskar', 'Viljuškar'),
     getDashboardTableHtml('baterija', 'Baterija')
   ].join('');
 }
...
function resetCalculatorForm(){
  ...
  if ($('reExportType')) $('reExportType').value = 'standard';
...
function loadCalculationIntoForm(item){
  ...
  try {
    ...
    const savedType = item.reExport ?? item.reExportType ?? 'standard';
    if ($('reExportType')) $('reExportType').value = String(savedType).toLowerCase() === 'reexport' || String(savedType).toLowerCase() === 're_export' || String(savedType).toLowerCase() === 're-export' ? 'reExport' : 'standard';
...
$('saveBtn').onclick=async ()=>{
 ...
 const reExportType = $('reExportType')?.value || 'standard';
 const item={...c,id:editingId||Date.now().toString().slice(-6),date:calcDate,commercialist,model:$('model').value,customer:$('customer').value,supplier:$('supplier').value,note:$('note').value,reExport:reExportType,reExportType,status:'Aktivna',createdBy: currentUser ? currentUser.username : ''};
...
if ($('search')) $('search').addEventListener('input', () => renderHistoryTable());
if (document.querySelectorAll('.history-filter').length) {
  document.querySelectorAll('.history-filter').forEach(btn => {
    btn.addEventListener('click', () => {
      historyFilter = btn.dataset.historyFilter || 'all';
      document.querySelectorAll('.history-filter').forEach(el => el.classList.toggle('active', el === btn));
      renderHistoryTable();
    });
  });
}
