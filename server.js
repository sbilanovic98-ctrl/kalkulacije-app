...
const legacyDbPath = path.join(__dirname, 'calculations.db');
...
function migrateLegacyDatabaseIfNeeded() {
  ...
  try {
    ...
    console.log(`Migracije: preneta legacy baza iz ${legacyDbPath} u ${dbPath}`);
  } catch (error) {
    console.warn('Ne mogu da prenesem legacy bazu:', error.message);
  }
}
...
function normalize(item = {}) {
  const rawType = item.reExport ?? item.reExportType ?? item.type ?? 'standard';
  const reExport = String(rawType).toLowerCase() === 'reexport' || String(rawType).toLowerCase() === 're_export' || String(rawType).toLowerCase() === 're-export' || String(rawType).toLowerCase() === 'true' ? 'reExport' : 'standard';
  return {
    ...item,
    reExport,
    reExportType: reExport,
  ...
  try {
    ...
    await runSql(
      `INSERT INTO calculations (
        ...
        productType, reExport, chargerWeight, chargerPrice, ecoCharger, customsCharger,
